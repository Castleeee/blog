...
![](./static/python语言进阶_images_4.png)
...
这个引用计数器需要保护,当多个线程同时修改这个值时,可能会导致内存泄漏;我们使用锁来解决这个问题,可有时会添加多个锁来解决,这就会导致**死锁**;  
为了避免内存泄漏和死锁问题,CPython使用了**单锁**,即全局解释器锁(GIL),即执行Python字节码都需要获取GIL,而其他线程如果想要操作和执行相同的代码需要等某个线程操作完了，释放了GIL后，这个线程才能拿到GIL锁，并且上锁，执行代码。
...
##### 大致实现原理
condition有两层锁，一把Rlock和一个waiters双端队列，用Rlock锁定要执行的块，每次wait都申请一把锁然后 ==acquire两次==放到waiters，每次notify就是拿出一把waiter锁release,每次with-cond都会进入enter把Rlock acquire，所以不用notify释放  
...
`action`是一个可调用对象， ==无参==，当线程被释放时，其中随机一个线程会首先调用`action`，之后再跑自己的代码。  
`timeout`时默认的超时时间。  
...
`abort()`
- 将障碍置为断开状态，导致正在等待或 ==之后调用==`wait()`引发`BrokenBarrierError`,注意和reset的区别  
...
`as_completed` 是一个yield生成器，接收线程池，yield ==已经完成的线程结果== 
...
#### 线程进程队列性能
|线程/进程/线程队列/进程队列| 10W数据进队列时间|10W数据出队列时间|100W数据进队列时间|100W数据出队列时间|结果分析|
|--|--|--|--|--|--|
|`collections.deque`|180ms|380ms|190ms|376ms|性能：高||
|`multiprocessing.Queue`|300ms|12s|1.92s|2m15s|性能：一般。|分析：进程队列为保证数据一致性，在操作数据的时候会有锁的操作|
|组合使用1：mq提供数据给collections.deque使用|300ms|11s|3.051s|2m26s|性能：一般。<br/>分析：结果跟直接使用进程队列相近，耗时主要发生在加锁释放锁上面
|组合使用2：一个进程作为生产者生产3个`multiprocessing.Queue`<br/>三个线程消费进程队列数据，存储到`collections.deque`|mq=300ms dq=50/s|mq=50/s dq=50/s|mq=3.0s dq=50/s|mq=dq=50/s|性能：差。<br/>分析：在线程中消费进程队列中的数据，可能会因为锁的抢占带来性能问题。<br/> 100W条数据mq入=6s,<br/> MQ抢占时段：mq出=dq入=dq出 = 50条/s， <br/>MQ非抢占时段:mq出=dq入=dq出 = 100W/50s。|

综合分析[^1]
-   MQ在数据量比较大的时候，抢占队列会导致出队列速度降低。
-   不推荐组合使用，建议使用进程multiprocessQ或都使用线程+collect.deque比较好。
...
:::
 ...
 ==多线程的锁和队列共享等在多进程都是不能用的==，但是在multiprocessing中有替代品
...
#TODO 细化多线程多进程的东西
多线程线程池的函数
- [Python concurrent.futures 的 as_completed 函数解释_潘广宇的博客-CSDN博客](https://blog.csdn.net/panguangyuu/article/details/105335900)
- [Python多线程使用concurrent.futures.as_completed方法下的一个坑_Grady.Lo的博客-CSDN博客](https://blog.csdn.net/u012183105/article/details/88909955)
...
#TODO  异步编程，asyncio asynchttp gevent twisted 
结合多进程和多线程
...
### loguru
loguru 是对logging的封装，logging属实难用，配置多又不安全，现在用得最多的就是第三方loguru，下面我会总结出我多用的场景  
#TODO 多线程多进程协程 锁的性能影响 分文件 日志格式 压缩 大小分片时间分片，滚动删除，cache exceptinon 
- [python日志记录loguru模块_写代码ing的博客-CSDN博客](https://blog.csdn.net/lhbo_bo/article/details/119359836)  
- [python loguru优雅的输出日志信息_darling331的博客-CSDN博客](https://blog.csdn.net/brightgreat/article/details/123437605)
...
[^1]: [collections.deque和multiprocessing.Queue组合使用带来的性能问题 - 掘金](https://juejin.cn/post/7152352643430481928)